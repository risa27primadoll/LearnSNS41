...
session_start();
 require("dbconnect.php");
  ...
  $sql = 'SELECT * FROM `users` WHERE `id`=?';
   ...
   $following_sql = "SELECT `fw`.*, `u`.`created`,`u`.`name`,`u`.`img_name` FROM `followers` AS `fw` LEFT JOIN `users` AS `u` ON `fw`.`follower_id` = `u`.`id` WHERE `user_id`=?";
...
//followerの一覧の取得
//今表示されているプロフィールページの人が、フォローされている
$follower_sql = "SELECT `fw`.*, `u`.`created`,`u`.`name`,`u`.`img_name` FROM `followers` AS `fw` LEFT JOIN `users` AS `u` ON `fw`.`user_id` = `u`.`id` WHERE `user_id`=?";
